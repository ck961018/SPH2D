...
ArrayAccessor1<Vector2D> ParticleSystemData2::positions() { return vectorDataAt(_positionIdx); }
...
void ParticleSystemData2::addParticle(const Vector2D& newPosition, const Vector2D& newVelocity,
...
void ParticleSystemData2::buildNeighborLists(double maxSearchRadius) {
  ...
  for (size_t i = 0; i < numberOfParticles(); ++i) {
    ...
    _neighborSearcher->forEachNearbyPoint(origin, maxSearchRadius, [&](size_t j, const Vector2D&) {
      if (i != j) {
...
void ParticleSystemData2::deserializeParticleSystemData(
  ...
  _neighborSearcher = Factory::buildPointNeighborSearcher2(fbsNeighborSearcher->type()->c_str());
